...
External multi-site validation is necessary, but it is not enough on its own. Before deploying an AI system at a specific clinical site, local validation confirms that the system performs acceptably for the local patient population, with local equipment, and within the local clinical workflow.
