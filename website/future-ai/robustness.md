...
The robustness principle holds that healthcare AI tools should maintain reliable performance and accuracy under expected and unexpected variations in real-world input data. A robust AI system does not fail catastrophically when confronted with inputs that differ from its training distribution.
...
Models trained and evaluated in controlled conditions frequently demonstrate high performance, then drop substantially in real-world deployment. This is the robustness gap, and it is well documented in medical AI.
